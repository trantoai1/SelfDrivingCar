...
def binarization(image):  # ảnh trắng đen(ảnh nhị phân)
    ...
    if Setting.HSVBARENABLE:
        ...
        # cv2.imshow('thresh2', binary_image)
    else:
...
def preprocess_image(image):  # tiến trình xử lý ảnh
    image = constrastLimit(image)  # giới hạn độ tương phản
    showImage(image, 'constrastLimit')
    image = LaplacianOfGaussian(image)  # lọc biên
    showImage(image, 'LaplacianOfGaussian')
    image = binarization(image)  # chuyển sang ảnh trắng đen để dễ xử lý
    showImage(image, 'binarization')
    return image


# Find Signs
def removeSmallComponents(image, threshold):  # loại bỏ nhiễu dựa vào ngưỡng ảnh
...
def localization(image, min_size_components=300, similitary_contour_with_circle=0.65, count=0, current_sign_type=None):
...
def main(args):
    ...
    vidcap = cv2.VideoCapture(Setting.CAMERASIGN[Setting.CAMERAMODE])
